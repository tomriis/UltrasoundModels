function [Th] = horizontal_array(n_elements_r, n_elements_z, kerf, D_rz, R_focus,a,b)
    ...
    angle_r = linspace(0,2*pi,n_elements_r);
    ...
    for i = 1:length(angle_r)
        ...
        focused_rectangles([3,6,9,12,18],:)=focused_rectangles([3,6,9,12,18],:) - R_focus;
    ...
    % Position transducer
        ...
        rot = make_ellipse_z_rot_mat(angle_r(i),a,b);
        rect([3,6,9,12,18],:)=rect([3,6,9,12,18],:)+b/2;
        positioned_rect = apply_affine_to_rect(rot, rect);
    % Append to transducer geometry
